...
const Navbar = () => {
  ...
  return (
    <motion.nav
      className="w-full flex justify-between items-center navbar py-4 px-6 bg-white/80 backdrop-blur-xl shadow-lg rounded-2xl sticky z-50 border border-secondary-light/25 transition-all duration-300"
    ...
    >
      ...
      >
        <img
          ...
          className="w-[50px] h-[50px] rounded-full border-2 border-secondary-light"
        />
        <div className="hidden sm:block min-w-0 max-w-[180px] ss:max-w-[240px] md:max-w-[340px] lg:max-w-none flex-shrink">
          <h3 className="font-poppins font-bold text-xs ss:text-sm md:text-base lg:text-lg text-textPrimary-light truncate" title={TITLE_LINE}>
            {TITLE_LINE}
          </h3>
        </div>
      </a>
            ...
            <a
              ...
              className={`relative group px-4 py-2 rounded-lg transition-all duration-300 ${
                activeSection === nav.id
                  ...
                  : "text-textPrimary-light hover:text-secondary"
              }`}
      ...
      <div className="sm:hidden flex flex-1 justify-end items-center gap-3">
        ...
        <motion.div
          className={`${
            toggle ? "flex" : "hidden"
          } p-6 bg-white/95 backdrop-blur-xl absolute top-20 right-0 mx-4 my-2 min-w-[200px] rounded-2xl shadow-xl border border-secondary-light/25 sidebar`}
                ...
                <a
                  ...
                  className={`block px-4 py-2 rounded-lg transition-all duration-300 ${
                    activeSection === nav.id
                      ...
                      : "text-textPrimary-light hover:text-secondary hover:bg-secondary/10"
                  }`}
                >
                  {nav.title}
                </a>
